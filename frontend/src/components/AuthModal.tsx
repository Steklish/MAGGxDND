...
export const AuthModal: React.FC<AuthModalProps> = ({ mode, onClose, onRegisterSuccess, onLoginSuccess }) => {
    ...
    useEffect(() => {
        // Try to fetch OAuth config from backend
        fetch('/api/v1/oauth/config')
            .then(res => res.json())
            .then(data => {
                setOauthConfigured({
                    google: data.google_configured || false,
                    discord: data.discord_configured || false
                });
            })
            .catch(err => {
                console.warn('Could not fetch OAuth config:', err);
                setOauthConfigured({ google: false, discord: false });
            });
    }, []);
